...
export class CollisionSystem extends Birch.World.System {
	...
	update(): void {
		...
		for (const entry of entities) {
			...
			if (physics !== undefined && frame !== undefined) {
				...
				min.set(Birch.Num.clamp(Math.floor(frame.position.x - physics.radius), 0, map.size.x - 1),
					Birch.Num.clamp(Math.floor(frame.position.y - physics.radius), 0, map.size.y - 1));
				max.set(Birch.Num.clamp(Math.floor(frame.position.x + physics.radius), 0, map.size.x - 1),
					Birch.Num.clamp(Math.floor(frame.position.y + physics.radius), 0, map.size.y - 1));
